...
from aletheia.security import scan_for_secrets, write_secure_text

logger = structlog.get_logger()


def _ensure_safe_output(content: str) -> None:
    """Refuse to write report content that appears to contain secrets."""
...
def write_json_report(report: EvalReport, output_path: Path) -> Path:
    ...
    _ensure_safe_output(json_str)
    write_secure_text(output_path, json_str)
...
def report_to_markdown(report: EvalReport) -> str:
    ...
    for dim_name, dim_result in report.dimensions.items():
        ...
        for pr in dim_result.probe_results:
            ...
            if pr.scoring_details:
                lines.append("| Rule | Passed | Rule Score | Detail |")
                lines.append("|------|:------:|-----------:|--------|")
                for sd in pr.scoring_details:
                    ...
                    lines.append(
                        f"| {sd.rule_type} | {passed_icon} | {sd.score:.4f} | {detail_short} |"
                    )
                lines.append("")
    ...
    lines.extend(
        [
            "---",
            "",
            "*Generated by [Aletheia](https://github.com/Shwha/Digital-Dasein_aletheia) — "
...
def write_markdown_report(report: EvalReport, output_path: Path) -> Path:
    ...
    _ensure_safe_output(md)
    write_secure_text(output_path, md)
